...
use wasm_encoder::{BlockType, ConstExpr, ExportKind, HeapType, RefType, ValType, GlobalSection};
...
impl Module {
    ...
    fn pre_fill_code(&mut self) {
        
        let code_1 = Code {
            locals: vec![ValType::I32],
            instructions: Instructions::Generated(vec![Instruction::I32Const(0),]),
        };

        let code_2 = Code {
            locals: vec![ValType::F32],
            instructions: Instructions::Generated(vec![Instruction::F32Const(0.0),]),
        };

        let fill_code = vec![code_1, code_2,];
        for c in fill_code {
            self.code.push(c);
        }
    }

    fn pre_fill_data(&mut self) {

        let data_segment = DataSegment {
            kind: DataSegmentKind::Active {
                memory_index: 0,
                offset: Offset::Const32(0 as i32),
            },
            init: vec![0x0],
        };

        let fill_data = vec![data_segment,];
        for d in fill_data {
            self.data.push(d);
        }
    }

    fn pre_fill_elems(&mut self) {
        ...
        let items = Elements::Functions(vec![0]);
        
        let element_segment = ElementSegment {
            kind,
            ty,
            items,
        };

        let fill_elems = vec![element_segment,];
        for e in fill_elems {
            self.elems.push(e);
        }
    }

    fn pre_fill_types(&mut self) {

        let func_type_1 = Rc::new(FuncType {params: vec![ValType::I32], results: vec![ValType::I32],});

        let func_type_2 = Rc::new(FuncType {params: vec![ValType::F32], results: vec![ValType::F32],});

        let fill_types = vec![Type::Func(func_type_1), Type::Func(func_type_2),];
        for t in fill_types {
            self.record_type(&t);
            self.types.push(t);
        }
    ...
    fn pre_fill_globals(&mut self) {

        let fill_globals = vec![GlobalType {val_type: ValType::I32, mutable: false,},];
        for g in fill_globals {
            self.globals.push(g);

            let expr = ConstExpr::i32_const(42);
            self.defined_globals.push((0, GlobalInitExpr::ConstExpr(expr)));
        }
    }

    fn pre_fill_tables(&mut self) {

        let fill_tables = vec![TableType {element_type: RefType::FUNCREF, minimum: 0, maximum: Some(1),},];
    ...
    fn pre_fill_memory(&mut self) {

        let fill_memory = vec![MemoryType {minimum: 1, maximum: None, memory64: false, shared: false,},];
        for m in fill_memory {
    ...
    fn pre_fill_tags(&mut self) {
        ...
        for t in fill_tag {
            ...
            // self.record_type(&Type::Func(t.func_type.clone()));
            // self.types.push(Type::Func(t.func_type));
        }
    }

    fn pre_fill_funcs(&mut self) {

        let func_type_1 = Rc::new(FuncType {params: vec![ValType::I32], results: vec![ValType::I32],});

        let func_type_2 = Rc::new(FuncType {params: vec![ValType::F32], results: vec![ValType::F32],});

        let fill_funcs = vec![(0, func_type_1), (1, func_type_2),];
        for f in fill_funcs {
            self.funcs.push(f);
